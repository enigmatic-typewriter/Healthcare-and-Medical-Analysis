...
def clean_dataframe(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    data["smoking_history"] = data["smoking_history"].fillna("No Info").astype(str).str.strip().replace({"nan": "No Info", "N/A": "No Info"})
...
def run_training_workflow(data: pd.DataFrame) -> dict:
    ...
    for model_name, estimator in MODEL_CATALOG.items():
        ...
        leaderboard_rows.append({"model": model_name, **{k: v for k, v in metrics.items() if k != "confusion_matrix"}})
    ...
    importance = permutation_importance(best_model, x_test, y_test, n_repeats=4, random_state=42, scoring="f1", n_jobs=1)
    feature_importance = (
